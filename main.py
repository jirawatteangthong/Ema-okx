...
def add_trade_result(side: str, reason: str, contracts: int, entry_px: float, exit_px: float, contract_size: float):
    """reason: 'TP' | 'SL' | 'MANUAL' ; Step3 จะถูกนับเป็น 'TP' โดยลอจิกตอนปิดโพซิชันแล้ว"""
    price_diff = (exit_px - entry_px) if side == 'long' else (entry_px - exit_px)
    pnl_per_contract = price_diff * contract_size
    pnl_total = pnl_per_contract * contracts
...
# ================== LOG HELPERS ==================
def log_tick_status(armed_side_like, f_now, s_now, in_pos, pos_side, price):
    try:
        side_txt = 'NONE' if armed_side_like is None else str(armed_side_like).upper()
        if not in_pos:
...
# ================== MAIN (Cross-entry + SL 3 ขั้น + TP + Alerts + Monthly Stats) ==================
if __name__ == "__main__":
    ...
    # 👉 ใช้เป็นตัวควบคุม "รอการตัดครั้งใหม่"
    ...
    while True:
        try:
            ...
            # ===== ตั้ง last_cross_dir ครั้งแรก (กันเข้าโดยยังไม่เกิดการตัดใหม่) =====
            if last_cross_dir is None and not in_pos:
                if f_now > s_now:
                    last_cross_dir = 'long'
                elif f_now < s_now:
                    last_cross_dir = 'short'
                else:
                    last_cross_dir = None
