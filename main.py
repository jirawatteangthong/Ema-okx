...
# ---- SL เริ่มต้นจาก High/Low 50 แท่ง (soft SL ในบอท) ----
SWING_LOOKBACK_M5   = 50         # ใช้ช่วง 50 แท่งล่าสุด (M5)
SL_EXTRA_POINTS     = 200.0      # buffer 200 จุด
MAX_INITIAL_SL_POINTS = 1234     # ไม่ให้ SL ไกลเกินจาก entry
...
TOUCH_EMA200_LIVE = True # True = แตะ EMA200 ใช้แท่งกำลังวิ่ง / False = ใช้แท่งปิดเหมือนเดิม
...
# =========== Helpers: recent min/max for SL ===========
def recent_min_max(ohlcv_m5, lookback=SWING_LOOKBACK_M5):
    """คืนค่า (lowest_low, highest_high) จาก 50 แท่ง 'ล่าสุดที่ปิดไปแล้ว' """
    if not ohlcv_m5 or len(ohlcv_m5) < lookback + 2:
        look = ohlcv_m5[:]
    else:
        look = ohlcv_m5[:-1][-lookback:]   # ใช้เฉพาะแท่งปิดแล้ว
    highs = [c[2] for c in look] if look else []
    lows  = [c[3] for c in look] if look else []
...
def open_market(side: str, price_now: float):
    ...
    try:
        ...
        # ================== SL เริ่มต้น (ตาม requirement ใหม่) ==================
        # ใช้ highest high / lowest low จาก 50 แท่งล่าสุดที่ปิดไปแล้ว แล้วบวก/ลบ buffer 200 จุด
        ohlcv_m5 = exchange.fetch_ohlcv(SYMBOL_U, timeframe=TIMEFRAME_M5, limit=max(SWING_LOOKBACK_M5 + 5, 60))
        low50, high50 = recent_min_max(ohlcv_m5, SWING_LOOKBACK_M5)

        if low50 is None or high50 is None:
            # กันกรณีข้อมูลไม่พอ → fallback แบบเดิม (หา swing แล้วบวก/ลบ 200)
            highs = [c[2] for c in (ohlcv_m5[:-1] if ohlcv_m5 else [])][-SWING_LOOKBACK_M5:] or [entry_ref]
            lows  = [c[3] for c in (ohlcv_m5[:-1] if ohlcv_m5 else [])][-SWING_LOOKBACK_M5:] or [entry_ref]
            low50, high50 = (min(lows), max(highs))

        if side == 'long':
            # ต้องต่ำกว่า lowest low 50 แท่งลงอีก 200 จุด
            raw_sl = float(low50) - float(SL_EXTRA_POINTS)
            # อย่าให้ไกลเกิน MAX_INITIAL_SL_POINTS
            sl0 = max(raw_sl, entry_ref - MAX_INITIAL_SL_POINTS)
        else:
            # ต้องสูงกว่า highest high 50 แท่งขึ้นอีก 200 จุด
            raw_sl = float(high50) + float(SL_EXTRA_POINTS)
...
def send_startup_banner():
    try:
        ...
        send_telegram(
            ...
            f"🛡 SL เริ่มต้นจาก High/Low {SWING_LOOKBACK_M5} แท่ง ±{int(SL_EXTRA_POINTS)} pts (≤ {int(MAX_INITIAL_SL_POINTS)} pts)\n"
...
def main():
    ...
    # ✅ โหมดทดลอง: ข้าม H1 cross แล้วไปหาเข้า M5 ทันที
    if START_FORCE_PLAN in ('long', 'short'):
        cur_dir, h1_ts, _ = get_h1_dir_closed()
        entry_plan.update({
            'h1_dir': START_FORCE_PLAN,
            'h1_bar_ts': h1_ts,
            'stage': 'armed',
            'm5_last_bar_ts': None,
            'm5_touch_ts': None,
        ...
        send_telegram(
            "🧪 <b>TEST MODE</b>: ข้าม H1 cross → เริ่มหาเข้า M5 ทันที\n"
